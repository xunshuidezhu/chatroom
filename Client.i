...
void Client::Connect()
{
    cout << "Connect server: " << "127.0.0.1" << " : " << 8080 << endl;
...
void Client::Start()
{
    ...
    if (pid < 0) {
        ...
        while (isClientWork) {
            bzero(&message, 0xFFFF);
            fgets(message, 0xFFFF, 
# 88 "Client.cpp" 3 4
                                    stdin
# 88 "Client.cpp"
                                         );
        ...
        while (isClientWork) {
            ...
            for (int i = 0; i < epoll_events_count; ++i) {
                ...
                if (events[i].data.fd == sock) {
                    int ret = recv(sock, message, 0xFFFF, 0);
                    if (ret == 0) {
                    ...
                    int ret = read(events[i].data.fd, message, 0xFFFF);
                    if (ret == 0){
                        isClientWork = 0;
                    }else{
                        send(sock, message, 0xFFFF, 0);
                    }
                }
            }
        }
